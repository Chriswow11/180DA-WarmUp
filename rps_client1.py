...
def on_message(client, userdata, message):
  ...
  if message.topic == "rps2":
    ...
    global player2_move
    received_data2 = True
    player2_move = message.payload.decode()
	...
	while (not received_data1 or not received_data2) and running:
		...
		for event in pygame.event.get():
			if event.type == KEYDOWN:
				if event.key == K_LEFT:
					if (player1_move == ""):
						client.publish("rps1", "rock", qos = 1)
					player1_move = "rock"
				elif event.key == K_UP:
					if (player1_move == ""):
						client.publish("rps1", "paper", qos = 1)
						print("p1 used paper")
						player1_move = "paper"
				elif event.key == K_RIGHT:
					if (player1_move == ""):
						client.publish("rps1", "scissors", qos = 1)
						print("p1 used scissors")
